...
lidar_body_topic: "/lidar_points_body"
lidar_optical_topic: "/lidar_points_optical"
lidar_world_topic: "/lidar_points_odom"
...
`/lidar_points` is retained only for compatibility and is deprecated: its values are
body-frame points while its historical header says `odom`. New consumers must use
`/lidar_points_body`, `/lidar_points_optical`, or `/lidar_points_odom`; each new
topic has point values and `frame_id` in the same coordinate frame. Depth and
CameraInfo share the `camera_optical` frame and timestamp.
