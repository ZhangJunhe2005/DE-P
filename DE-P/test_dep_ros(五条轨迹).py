...
from geometry_msgs.msg import PoseStamped, Point  # 确保Point导入
from cv_bridge import CvBridge
from threading import Lock
from sensor_msgs.msg import PointCloud2, PointField, Image
from sensor_msgs import point_cloud2
from visualization_msgs.msg import Marker  # 用于发布线条
...
class DepNet:
    def __init__(self, config, weight):
        ...
        # -------------------------- 1. 颜色配置接口（移至__init__，方便外部修改） --------------------------
        # 默认5种颜色，用户可直接修改RGBA值（范围0-1，A为透明度）
        self.traj_colors = [
            ColorRGBA(219.0/255.0, 40.0/255.0, 52.0/255.0, 0.8),    # 红色（最佳轨迹默认用此色）
            ColorRGBA(252.0/255.0, 152.0/255.0, 113.0/255.0,0.8),    # 黄色
            ColorRGBA(102.0/255.0, 193.0/255.0, 164.0/255.0, 0.8),    # 绿色
            ColorRGBA(76.0/255.0, 155.0/255.0, 230.0/255.0, 0.8),    # 蓝色
            ColorRGBA(249.0/255.0, 223.0/255.0, 145.0/255.0,0.8)    # 紫色
        ]
        ...
        # -------------------------- 2. 修正ROS发布器类型（PointCloud2 → Marker） --------------------------
        # 轨迹发布器：全部改为visualization_msgs/Marker类型（用于显示线条）
        self.lattice_traj_pub = rospy.Publisher("/dep_net/lattice_trajs_visual", PointCloud2, queue_size=1)
        self.best_traj_pub = rospy.Publisher("/dep_net/best_traj_visual", Marker, queue_size=1)
        self.all_trajs_pub = rospy.Publisher("/dep_net/trajs_visual", Marker, queue_size=1)
        # 控制指令发布器（保持不变）
    ...
    def visualize_trajectory(self, pred_score, pred_endstate):
        ...
        if self.best_traj_pub.get_num_connections() > 0:
            ...
            # 构建Marker消息（LINE_STRIP：连续线段）
            marker = Marker()
            marker.header = Header(stamp=rospy.Time.now(), frame_id='world')  # 显式初始化Header
            marker.id = 0  # 唯一ID，避免RViz中重复覆盖
            marker.type = Marker.LINE_STRIP  # 连续线段类型
            marker.action = Marker.ADD  # 添加/更新标记
            marker.pose.orientation.w = 1.0  # 无旋转（单位四元数）
            
            # 线条属性
            marker.scale.x = 0.1  # 线宽（最佳轨迹加粗，便于区分）
            marker.color = self.traj_colors[0]  # 用第一种颜色（默认红色）
            
            # 添加所有点到线段
            for point in points_array:
                marker.points.append(Point(x=point[0], y=point[1], z=point[2]))  # 显式指定x/y/z参数
        ...
        if self.visualize and self.lattice_traj_pub.get_num_connections() > 0:
            ...
            lattice_points_list = []
            intensities_list = []  # 存储所有点的强度值

            # 记录起点位置（用于计算距离）
            start_point = np.array([start_pos[0], start_pos[1], start_pos[2]])
            ...
            for i in range(num_lattice):
                ...
                # 计算当前轨迹所有点到起点的距离
                distances = np.linalg.norm(lattice_traj - start_point, axis=1)  # 形状：(20,)，每个点到起点的欧氏距离

                # 归一化距离到0~1范围（作为强度值，距离越远强度越大）
                if np.max(distances) > 0:  # 避免除以0
                    intensities = distances / np.max(distances)  # 归一化到0~1
                else:
                    intensities = np.zeros_like(distances)  # 若所有点与起点重合，强度为0
            ...
            # 合并所有轨迹的点和强度
            lattice_points_array = np.concatenate(lattice_points_list, axis=0)  # 形状：(N*20, 3)
            all_intensities = np.concatenate(intensities_list, axis=0)  # 形状：(N*20,)

            # 构造带强度通道的点云数据（x, y, z, intensity）
            points_with_intensity = np.column_stack((lattice_points_array, all_intensities))  # 形状：(N*20, 4)
            ...
            fields = [
                ...
                PointField(name='intensity', offset=12, datatype=PointField.FLOAT32, count=1)  # 强度通道
            ]

            # 创建带强度的点云消息并发布
            point_cloud_msg = point_cloud2.create_cloud(header, fields, points_with_intensity)
            self.lattice_traj_pub.publish(point_cloud_msg)
        
        # -------------------------- 3.3 所有预测轨迹（最多显示5条，对应5种颜色） --------------------------
        if self.visualize and self.all_trajs_pub.get_num_connections() > 0:
            t_values = np.arange(0, self.traj_time, dt)
            num_trajs = pred_endstate.shape[0]
            max_display = min(num_trajs, len(self.traj_colors))  # 最多显示与颜色数量相同的轨迹（默认5条）
            
            # 构建Marker消息（LINE_LIST：多条独立线段）
            marker = Marker()
            marker.header = Header(stamp=rospy.Time.now(), frame_id='world')
            marker.id = 2  # 与其他轨迹ID区分
            marker.type = Marker.LINE_LIST
            marker.action = Marker.ADD
            marker.pose.orientation.w = 1.0
            marker.scale.x = 0.06  # 线宽
            ...
            for i in range(max_display):
                ...
                # 为当前轨迹添加线段
                color = self.traj_colors[i]  # 用第i种颜色（一一对应）
                for j in range(len(traj_points) - 1):
                    marker.points.append(Point(x=traj_points[j][0], y=traj_points[j][1], z=traj_points[j][2]))
                    marker.points.append(Point(x=traj_points[j+1][0], y=traj_points[j+1][1], z=traj_points[j+1][2]))
                    marker.colors.append(color)
                    marker.colors.append(color)
            
            self.all_trajs_pub.publish(marker)
...
if __name__ == "__main__":
    ...
    weight = "dep_trt.pth" if args.use_tensorrt else base_dir + "/saved/DEP_{}/epoch{}.pth".format(args.trial, args.epoch)
