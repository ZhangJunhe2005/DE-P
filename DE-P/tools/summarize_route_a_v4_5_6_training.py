#!/usr/bin/env python3
"""Compact progress and result summary for V4.5.6."""
...
RUN_ROOT = ROOT / "runs/route_a_static_yopo_v4_5_6_continuous_score_safety"
...
def selected_metrics(value):
    names = (
        ...
        "clearance_pairwise_ranking_loss", "coarse_false_safe_candidate_count",
        "dense_clearance_drop",
    )
    return {
        name: float(value[name]) for name in names if name in value
    }
...
def main():
    ...
    result = {
        ...
        "role": "focused_continuous_safety_score_calibration",
        "run": str(run),
