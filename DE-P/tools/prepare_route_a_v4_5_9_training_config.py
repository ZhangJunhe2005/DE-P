#!/usr/bin/env python3
"""Prepare the V4.5.9 time-mean localized-safety shakedown."""
...
OUTPUT = ROOT / "configs/route_a_v4_5_9_time_mean_safety_shakedown.yaml"
PARENT_RUN_ROOT = ROOT / "runs/route_a_static_yopo_v4_5_6_continuous_score_safety"
...
def main():
    ...
    config["experiment_role"] = "time_mean_localized_safety_shakedown"
    ...
    config["output_root"] = str(
        ROOT / "runs/route_a_static_yopo_v4_5_9_time_mean_safety"
    )
    config["model"].update({
    ...
    config["static_yopo_v4_5_8"] = {"enabled": False}
    config["static_yopo_v4_5_9"] = {
        "enabled": True,
        ...
        "far_decay_m": 0.10,
        # A physically safe 0.30--0.50 m clearance remains visible to the
        # gradient but never dominates goal progress merely for being narrow.
    ...
    config["validation"].update({
        "contract_version": "route_a_v4_5_9_time_mean_localized_safety_v1",
        "minimum_epoch": 1,
        "patience": 5,
    })
    config["implementation_hotfixes"] = [
        "route_a_v4_5_9_initialize_from_noncollapsed_v456_best_v1",
        "route_a_v4_5_9_restore_original_yopo_time_mean_safety_v1",
        "route_a_v4_5_9_weak_safe_side_warning_band_v1",
        "route_a_v4_5_9_runtime_physical_collision_floor_unchanged_v1",
        "route_a_v4_5_9_keep_certified_candidate_retiming_v1",
        "route_a_v4_5_9_no_new_qualification_gate_v1",
    ]
    if OUTPUT.exists():
