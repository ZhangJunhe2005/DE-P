...
class StateTransform:
    ...
    def normalize_obs(self, vel_acc_goal):
        # Keep the caller-owned physical state intact.  The training objective
        # reuses velocity, acceleration and goal after network inference to
        # construct world-frame trajectory losses.  Mutating this tensor here
        # silently changed that physical state into normalized network units.
        velocity = vel_acc_goal[:, 0:3] / self.lattice_primitive.vel_max
        acceleration = vel_acc_goal[:, 3:6] / self.lattice_primitive.acc_max
        goal = vel_acc_goal[:, 6:9]

        # Clamp the goal direction to unit length.  Build the result without
        # slice assignment so this boundary remains valid if an upstream
        # caller differentiates with respect to the input state.
        goal_norm = goal.norm(dim=1, keepdim=True)
        goal = goal / goal_norm.clamp(min=self.goal_length)
        return torch.cat((velocity, acceleration, goal), dim=1)
