"""Semantically exact fast-path counterpart of PECR1 mapper v3."""
...
CONTRACT_VERSION = "provisional_outcome_mapper_fast_v1"


class ProvisionalOutcomeMapperFastV1:
    def __init__(self, pds_contract, evidence_contract, runtime_contract=None):
        self.frozen_mapper = ProvisionalOutcomeMapperV1(pds_contract)
        capacity = int((runtime_contract or {}).get(
            "diagnostic_ring_capacity", 256
        ))
        self.authorizer = ProvisionalEvidenceAuthorizerFastV1(
            evidence_contract, capacity
        )
        self.last_authorization = None
        self.last_birth_contract = None

    def map(self, outcome: Mapping, state_id, generation=0,
    ...
    def map_batch(self, rows):
        return tuple(self.map(*row) for row in rows)


__all__ = ["CONTRACT_VERSION", "ProvisionalOutcomeMapperFastV1"]
