"""V4.5.3 continuous clearance calibration and stable score supervision.

This objective keeps the V4.5.2 trajectory, kinematic, and relative-score
contracts.  It adds one differentiable minimum-clearance barrier to the same
candidate total cost learned by both the proposal generator and score head.
No candidate is rejected and no qualification Gate is introduced.
"""
...
class StaticYOPOParityConfigV453:
    enabled: bool = False
    ...
    def from_mapping(cls, value):
        ...
        if unknown:
            raise ValueError(f"unknown static_yopo_v4_5_3 keys: {unknown}")
        result = cls(**value)
        result.validate()
        return result
    ...
    def as_v452(self):
        ...
        return StaticYOPOParityConfigV452(**values)

    def contract(self):
        return {
            "version": "static_yopo_original_parity_v4_5_3",
            "parent": "static_yopo_original_parity_v4_5_2",
            "candidate_objective": (
                "v4_5_2_total_plus_continuous_minimum_clearance_barrier"
            ),
            "score_label": "detached_within_sample_standardized_same_total_cost",
            "score_calibration": "relative_regression_plus_one_listwise_kl",
            "training_schedule": "score_only_then_low_lr_joint_finetune",
...
def continuous_clearance_barrier_v4_5_3(
    minimum_clearance, *, vehicle_radius, weight, softness,
):
    """Unbounded smooth penalty for entering the physical vehicle envelope.

    The penalty is already active, but rapidly decays, just outside the body
    radius.  It never rejects a candidate and therefore cannot seal a narrow
    passage as a Boolean inflated-boundary check would.
    """
    if float(vehicle_radius) <= 0.0:
    ...
    normalized_deficit = (
        float(vehicle_radius) - minimum_clearance
    ) / float(softness)
    return float(weight) * F.softplus(normalized_deficit)


def static_yopo_parity_objective_v4_5_3(
    *, sampler, safety_loss, fixed, predicted, predicted_scores, goal_world,
    ...
    v452_total = candidates("candidate_raw_total_cost")
    clearance_barrier = continuous_clearance_barrier_v4_5_3(
        minimum_clearance,
        vehicle_radius=config.vehicle_radius_m,
        weight=config.clearance_barrier_weight,
        softness=config.clearance_barrier_softness_m,
    )
    candidate_total = v452_total + clearance_barrier
    ...
    result.update({
        ...
        "clearance_barrier_loss": clearance_barrier.mean(),
        "score_label": relative_label.reshape(-1),
        "candidate_clearance_barrier_cost": clearance_barrier.reshape(-1),
        ...
        "per_sample_clearance_barrier_loss": clearance_barrier.mean(dim=1),
