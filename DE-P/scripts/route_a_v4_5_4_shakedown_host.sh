...
CONFIG="$ROOT/configs/route_a_v4_5_4_independent_score_shakedown.yaml"
DRY_RUN=0
RESUME=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
conda run --no-capture-output -n yopo \
  python tools/prepare_route_a_v4_5_4_training_config.py
conda run --no-capture-output -n yopo python -m pytest -q \
  tests/test_static_yopo_parity_v4_5_2.py \
  tests/test_static_yopo_parity_v4_5_4.py \
