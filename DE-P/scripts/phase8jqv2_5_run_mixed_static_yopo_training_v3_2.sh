...
CONFIG="$ROOT/configs/phase8jqv2_5_mixed_static_yopo_training_v3_2.yaml"
VERIFY_ONLY=false
DRY_RUN=false
AUTHORIZED=false
RESUME=""
...
if find "$ROOT/runs/phase8_mixed_static_yopo_v3_2_low_lr_adamw" \
  -name training.lock -type f -print -quit 2>/dev/null | grep -q .; then
  echo "An active v3.2 training.lock already exists." >&2
  exit 2
fi
...
read -r -p "Type TRAIN_MIXED_STATIC_YOPO_V3_2 to continue: " CONFIRMATION
[[ "$CONFIRMATION" == "TRAIN_MIXED_STATIC_YOPO_V3_2" ]] || {
