...
CONFIG="$ROOT/configs/route_a_v4_2_7_recovery_split_training.yaml"
RESUME_CHECKPOINT=""
DRY_RUN=0
while [[ $# -gt 0 ]]; do
  case "$1" in
...
conda run --no-capture-output -n yopo \
  python tools/prepare_route_a_v4_2_7_recovery_split_training_config.py
conda run --no-capture-output -n yopo python -m pytest -q \
  tests/test_static_yopo_recovery_state_v2.py \
  tests/test_static_yopo_feasibility_score_v1.py \
  tests/test_phase8j_coverage_objective.py \
  tests/test_deadlock_recovery_v2.py \
