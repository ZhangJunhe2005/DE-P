...
CONFIG="$ROOT/configs/route_a_v4_2_8_goal_mobility_training.yaml"
RESUME_CHECKPOINT=""
DRY_RUN=0
while [[ $# -gt 0 ]]; do
  case "$1" in
...
conda run --no-capture-output -n yopo \
  python tools/prepare_route_a_v4_2_8_goal_mobility_training_config.py
conda run --no-capture-output -n yopo python -m pytest -q \
  tests/test_static_yopo_goal_progress_v2.py \
