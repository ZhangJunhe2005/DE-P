...
CONFIG="$ROOT/configs/route_a_v4_8_3_candidate_only_shakedown.yaml"
DRY_RUN=0
RESUME=""
...
conda run --no-capture-output -n yopo \
  python tools/prepare_route_a_v4_8_3_training_config.py
conda run --no-capture-output -n yopo python -m pytest -q \
  tests/test_route_a_v4_8_3_training_entry.py \
