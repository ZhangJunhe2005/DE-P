...
RUN_ROOT="$ROOT/runs/phase8_mixed_static_yopo_v3_3_balanced_plateau"
RUN_DIR="${1:-}"

if [[ -z "$RUN_DIR" && -d "$RUN_ROOT" ]]; then
...
[[ -n "$RUN_DIR" && -d "$RUN_DIR" ]] || {
  echo "No v3.3 training run found." >&2
  exit 2
}
...
[[ -f "$RUN_DIR/checkpoints/best.pth" ]] && \
  echo "best_checkpoint: $RUN_DIR/checkpoints/best.pth"
