#!/usr/bin/env bash
set -euo pipefail

ROOT="/home/zjh/YOPO/DE-P"
TARGET="${1:-full}"
if [[ "$TARGET" == "probes" ]]; then
  exec /home/zjh/miniconda3/envs/yopo/bin/python \
    "$ROOT/tools/summarize_route_a_v4_5_probes.py"
fi
case "$TARGET" in
  full) RUN_ROOT="$ROOT/runs/route_a_static_yopo_v4_5_bounded_danger" ;;
  cave|forest|pillar|room|wall)
    RUN_ROOT="$ROOT/runs/route_a_static_yopo_v4_5_probe_${TARGET}" ;;
  *) echo "Usage: $0 [full|probes|cave|forest|pillar|room|wall]" >&2; exit 2 ;;
esac

LATEST="$(find "$RUN_ROOT" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -n 1 || true)"
if [[ -z "$LATEST" ]]; then
  echo '{"status":"NOT_STARTED"}'
  exit 0
fi
echo "run_dir: $LATEST"
[[ -f "$LATEST/run_state.json" ]] && cat "$LATEST/run_state.json"
if [[ -f "$LATEST/metrics.jsonl" ]]; then
  echo
  echo "latest_epoch_metrics:"
  tail -n 1 "$LATEST/metrics.jsonl"
fi
[[ -f "$LATEST/checkpoints/best.pth" ]] && \
  echo "best_checkpoint: $LATEST/checkpoints/best.pth"
[[ -f "$LATEST/training_complete.json" ]] && {
  echo "training_complete:"
  cat "$LATEST/training_complete.json"
}
