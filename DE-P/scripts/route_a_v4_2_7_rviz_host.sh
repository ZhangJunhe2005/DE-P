...
RUN_ROOT="$ROOT/runs/route_a_static_yopo_v4_2_7_recovery_split"
SCENE="${1:-forest}"
...
if [[ -z "$LATEST" || ! -f "$LATEST/checkpoints/$CHECKPOINT_NAME" ]]; then
  echo "No hard-safety-qualified V4.2.7 checkpoint found under $RUN_ROOT" >&2
  echo "For diagnostic testing only, add --allow-unqualified." >&2
  exit 1
fi
if [[ "$CHECKPOINT_NAME" == "best_unqualified.pth" ]]; then
  echo "WARNING: loading Gate-unqualified V4.2.7 checkpoint for diagnostics." >&2
fi
